...
// （起＝挨拶、承＝定期報告とその深掘り、転＝話題転換、結＝締めの一言）。
...
function buildInterBotFollowUpHint() {
  ...
  if (interBotState.willBeFinalSend()) {
    return `${scene}\n\n直前の${counterpart}の発言を受けつつ、今日のところはこの対話を締めくくってください（起承転結の「結」）。新しい話題を広げたり新たな問いを重ねたりはせず、これまで話していた内容に一言だけ触れて区切りをつけ、この定期報告・意見交換の場を終える一言を添えてください。次の機会（また顔を合わせること）をさりげなく示唆する一言があってもよいですが、大げさな別れの挨拶にはしないこと。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
  }

  if (!canSwitchTopic) {
    return `${scene}\n\n直前の${counterpart}の発言を受けて、今話している話題をそのまま掘り下げる形で応答してください（起承転結の「承」）。新しい話題へ飛躍しないこと。具体的な内容（数値・観察・反応など）を一つ深掘りするように。${counterpart}が話をそらそうとしたり、話を元に戻そうとしたりした場合は、それに素直に合わせてください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
  }
