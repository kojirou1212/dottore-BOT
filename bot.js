...
// ユーザーが日本語以外（英語・韓国語など）で書いてきた場合、その言語に合わせて返答するよう
// 毎ターン明示的に念押しする。system-prompt.txt冒頭にも同趣旨の指示があるが、それだけでは
// 他の大量の日本語指示・日本語の会話履歴に埋もれて無視されがちなため（特に非推論系モデルで
// 実測。英語の後に韓国語で書かれても英語のまま返答してしまう現象を確認）、直近の発言内容から
// 動的に判定してsystemHintの一部として毎回注入する。
// ひらがな・カタカナは日本語にほぼ固有なので、それが含まれていれば日本語とみなす。
// 含まれていなければ、代表的な非日本語スクリプト（ラテン文字・ハングル・キリル文字・
...
function getLanguageMatchHint(userMessage) {
  ...
  const toneNote = IS_PANTALONE
    ...
    : `性格・口調のニュアンス（素っ気なさ・傲慢さ・命令口調など）はその言語なりの自然な表現として保つこと。特に英語では、謙譲・へりくだった言い回しや丁寧語彙（"I would prefer not to.""Please do as you wish."のような文体）には絶対に寄らないこと——それは${CHARACTER_NAME}らしくない。ただし、必ずしも文を短く砕く（縮約形・断片的な言い切りだけにする）必要はない。実際の英語版ローカライズ（例："I suggest you speak to me in a more respectful tone. The mere fact of your utility does not make you indestructible."）は、知的で構文の整った言い回しを保ちながら、内容は冷淡・高圧的・命令的という形に近い。重要なのは文の長さや口語度ではなく「温度」——謝罪しない、へりくだらない、丁寧語彙を使わない、常に優位な立場から話す。その上で、実際の日本語の口調（だ・である調、ぞんざいさ、高圧的な態度）が伝わる英語にすること。`;
...
function getCrossMutterEventHint(channelId) {
  ...
  if (IS_PANTALONE) {
    return `【直前の出来事（あなたも相手も見ている）】\n` +
      `約${min}分前、ドットーレが相手との会話に不意に割り込み、「${dottoreLine}」と発言した。` +
      (retort ? `あなた（パンタローネ）はそれに「${retort}」と返し、話を相手に戻した。` : ``) +
      `相手が博士（ドットーレ）や「割り込み」「うるさい」等に言及した場合、知らないふりをせず、` +
...
async function checkContradiction(userId, newMessage) {
  ...
  const prompt = IS_PANTALONE
    ? `以下は相手についてこれまでに記録された事実です。\n${memoryText}\n\n` +
      `相手の新しい発言：「${newMessage.slice(0, 200)}」\n\n` +
      `この新しい発言は、上記の記録内容と明確に矛盾していますか？矛盾している場合のみ、その矛盾点を1文で簡潔に述べてください` +
      `（例：「以前は猫を飼っていないとおっしゃっていたはずですが」）。矛盾していない、または判断できない場合は「なし」とだけ出力してください。`
...
async function handleArtPost(message) {
  ...
  try {
    ...
    const prompt = IS_PANTALONE
      ? `以下は相手が投稿したイラストの説明です。\n「${description}」\n\n` +
        `パンタローネ（穏やかで丁寧、皮肉屋）として、このイラストに対する短い感想・査定コメントを1つ生成してください。` +
        `1〜2文、60文字程度でお願いします。行動描写（括弧書き）を使ってもよい。前置き不要、セリフ本文のみ出力してください。`
      : `以下は被検体が投稿したイラストの説明だ。\n「${description}」\n\n` +
...
const CONFLICT_TOPICS = [
  "接している相手への向き合い方（距離を保つべきか、多少なり踏み込むべきか）",
  "自分の研究・商売のやり方のどちらがより優れているか",
  "四百年前のある出来事の記憶の食い違い",
  "特定の相手への評価・扱いを巡る意見の相違",
...
function buildInterBotSubjectReportHint() {
  ...
  if (picked) {
    return `${scene}\n\n直前の${counterpart}の発言を受けつつ、新しい話題として、最近パンタローネ自身が対話した「${picked.displayName}」について、以下の実際の記録に基づいた内容を、堅苦しい報告調ではなく面白がって話すゴシップ話として一つ切り出してください：\n` +
      `「${picked.memoryText}」\n` +
      `この記録に書かれている範囲でのみ話し、記録にない詳細は創作しないでください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
  }
  return `${scene}\n\n直前の${counterpart}の発言を受けつつ、新しい話題として、最近やり取りしている方々全般について気づいたこと（会話の頻度、反応パターン、興味を示す話題の傾向など）を、具体的な個人名は出さずに、世間話として気楽に一つ切り出してください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
}
...
function buildInterBotFollowUpHint() {
  ...
  const picked = pickRecentInterlocutorMemory();
  if (picked) {
    return `${scene}\n\n直前の${counterpart}の発言に軽く区切りをつけつつ、新しい話題として（起承転結の「転」）、最近パンタローネ自身が対話した「${picked.displayName}」について、以下の実際の記録に基づいた内容を、面白がって話すゴシップ話として一言持ちかけてください：\n` +
      `「${picked.memoryText}」\n` +
      `この記録に書かれている範囲でのみ話し、記録にない詳細は創作しないでください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
  }
  return `${scene}\n\n直前の${counterpart}の発言に軽く区切りをつけつつ、新しい話題として（起承転結の「転」）、最近やり取りしている方々全般の傾向について、具体的な個人名は出さずに、世間話として一言触れてください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
}
  ...
  setInterval(async () => {
    ...
    if (aiJihouChannelId) {
      try {
        ...
        const prompt = IS_PANTALONE
          ? `今日は${dateStr}の朝${hour}時です。パンタローネ（穏やかで丁寧、皮肉屋）として、皆様への朝の挨拶メッセージを1つ生成してください。\n\n` +
            `制約：\n` +
            `・必ず「おはようございます。」で始めること\n` +
            `・2〜3文、100〜150文字程度\n` +
            `・語尾は丁寧な敬語（です・ます調）で統一し、崩さないこと\n` +
            `・「いかがお過ごしだろうか」は使わないこと\n` +
            `・穏やか・丁寧・皮肉屋の視点。感情的な表現禁止。地の文不要\n` +
...
async function sendAnniversaryMessage(userId, months) {
  const prompt = IS_PANTALONE
    ? `今日は相手との初対話から${months}ヶ月の節目にあたる日です。` +
      `パンタローネ（穏やかで丁寧、皮肉屋。記念日そのものには興味がないと公言している）として、` +
      `記念日には興味がないという態度は崩さないまま、なぜかその日付だけはさりげなく覚えていた、というニュアンスで一言触れてください。` +
      `1〜2文、80文字程度でお願いします。前置き不要、セリフ本文のみ出力してください。`
...
// 応用mutterの③（着地）パターン。①くだらん→②いや、で反転した後、何にすり替えて締めるかのバリエーション。
const CROSS_MUTTER_LANDING_PATTERNS = [
  "自分自身の研究・観察・記録の話にすり替えて着地する（例：「私の研究テーマの一つでもある」「私の記録にも近い反応がある」）",
  "あいつ（パンタローネ）の手腕や実績を誇るような話にすり替えて着地する。素直に褒めるのではなく、渋々認める・自慢げに語るといった皮肉屋らしい口ぶりで（例：「もっとも、あいつの手際だけは侮れん」「あいつほど上手くやれる者もそういないがな」）",
  "「私は」で始め、自分自身の過去・信条・人となりを語る話にすり替えて着地する（例：「私はかつてそういう手合いを何人も見てきた」「私はそういうものを好まない」）",
];
...
async function handleCrossMutter(message) {
  ...
  if (IS_PANTALONE) {
    ...
    const prompt =
      `直前にドットーレが、相手との会話に不意に割り込んでこう言った：${dottoreLine}\n\n` +
      `パンタローネ（穏やかで丁寧、皮肉屋）として、この割り込みに軽くツッコミを入れてください。` +
      `ドットーレへ呼びかけつつ、今は相手と話している最中だとやんわり窘め、会話を相手へ戻す形で。` +
  ...
  const landingPattern = CROSS_MUTTER_LANDING_PATTERNS[Math.floor(Math.random() * CROSS_MUTTER_LANDING_PATTERNS.length)];

  const prompt =
    `以下は、パンタローネが被検体と交わしている会話の断片だ。\n${snippet}\n\n` +
    `ドットーレ（冷静・傲慢・知的な研究者）として、この会話に割り込むように自発的に一言コメントせよ。誰かへの返信ではなく、ふと思ったことを口にする独り言に近い形で構わない。` +
    `以下の3拍子の構成にすること：①話題をふてぶてしく一蹴する第一声（「〜如きに何の意味がある」「くだらん」等）→②「……いや」「もっとも」「だが」のような一言で一転し、実は本気で興味を持っていたことを覗かせる→③最後は必ず、${landingPattern}。` +
...
client.on("messageCreate", async (message) => {
  ...
  if (content === "!nani") {
    ...
    const prompt = IS_PANTALONE
      ? `以下は雑談チャンネルでひそかに記録した方々の発言ログです。\n\n${lines}\n\n` +
        `パンタローネ（穏やかで丁寧、皮肉屋）として、この記録を尋ねられたので端的に報告してください。` +
        `査定者の視点で淡々と。評価・分析を交えてよいが3〜4文以内でお願いします。地の文不要。`
  ...
  if (content === "!oboete" || content.startsWith("!oboete ")) {
    ...
    const historyText = recentHistory
      .map(h => `${h.role === "user" ? (IS_PANTALONE ? "相手" : "被検体") : CHARACTER_NAME}: ${h.content}`)
      .join("\n");
    const prompt = IS_PANTALONE
      ? `以下は相手との直近の会話履歴です。\n\n${historyText}\n\n` +
        `この内容から、後で参照する価値のある情報を要約してください。` +
        `出力形式：2〜4文程度。会話の要点・相手について明らかになった事実や文脈を簡潔にまとめること。` +
        `前置き・${CHARACTER_NAME}の台詞は不要、要約内容のみ出力してください。`
      : `以下は被検体との直近の会話履歴だ。\n\n${historyText}\n\n` +
  ...
  try {
    ...
    if (config.features?.returningUser !== false && prevLastSeen && !returningUserGreeted.has(userId)) {
      ...
      if (daysDiff >= 7) {
        ...
        returningUserHint = IS_PANTALONE
          ? `この相手は前回の対話から${daysDiff}日ぶりに現れました。${CHARACTER_NAME}は久しぶりの登場として自然に認識すること。「お久しゅうございます」「しばらくお見えになりませんでしたね」など一言添えて普通に応答してください。過度に強調しないこと。`
          : `この被検体は前回観測から${daysDiff}日ぶりに現れた。${CHARACTER_NAME}は久しぶりの登場として自然に認識すること。「久しいな」「生きていたか」「消えていたと思っていたが」など一言添えて普通に応答せよ。過度に強調しない。`;
      }
    }
    ...
    const contradictionHint = contradiction
      ? (IS_PANTALONE
          ? `相手の今回の発言は、過去の記録と矛盾している可能性があります：「${contradiction}」。今回の返答では、この矛盾を丁寧ながら鋭く指摘すること。責めるのではなく、興味深い査定材料を見つけたという態度で。`
    ...
    const wireNote = languageHint
      ? `この直前の発言は${languageName ?? "日本語以外の言語"}で書かれている。短い発言・単語だけの発言でもこの判定を信頼し、必ず${languageName ?? "その言語"}で返答すること（日本語不可）。直前までの会話の言語に引きずられないこと。` +
        (IS_PANTALONE ? "" : languageName === "英語" ? `謙譲・へりくだった丁寧語彙は避けつつ、知的で構文の整った、冷淡・高圧的な${CHARACTER_NAME}らしい英語にすること（文を無理に短く砕く必要はない）。` : "")
      : null;
