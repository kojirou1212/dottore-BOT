...
function buildInterBotReplyHint() {
  ...
  return IS_PANTALONE
    ? `${scene}\n\n直前の${counterpart}の発言に、自然に応答してください。素っ気なく返す場合でも、それだけで終わらせず、自分なりの具体的な見解や情報を一言添えてください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`
    : `${scene}\n\n直前の${counterpart}の発言に対して自然に応答せよ。素っ気なく皮肉げな態度は保ってよいが、ただ拒絶したり話をはぐらかしたりするだけで終わらせず、自分なりの具体的な見解・分析・情報を一言添えること（例：「根本はもっと退屈だ」で終わらせず、それが何を指すのか一言で示す）。1〜3文程度。普段の会話と同じ形式（括弧書きの動作描写を交えて構わない）で、セリフ本文を出力すること。`;
}
...
// パンタローネ専用：3往復目以降に使う話題。
// 以前は毎回「新しい話題として」持ちかける指示になっており、往復のたびに無関係な話題へ
// 飛躍してしまう不具合があった（実測で確認）。会話が不自然に忙しくなるため、話題の切り替えは
// セッション中1回・かつ会話がある程度進んでから（transcript長8以降）のみ許可し、それ以外は
// 直前までの話題をそのまま掘り下げる指示にする。
function buildInterBotFollowUpHint() {
  const scene = interBotSceneHint();
  const counterpart = interBotCounterpartName();
  const transcript = interBotState.getTranscript();
  const canSwitchTopic = transcript.length >= 8 && !interBotState.hasSwitchedTopic();

  if (!canSwitchTopic) {
    return `${scene}\n\n直前の${counterpart}の発言を受けて、今話している話題をそのまま掘り下げる形で応答してください。新しい話題へ飛躍しないこと。具体的な内容（数値・観察・反応など）を一つ深掘りするように。${counterpart}が話をそらそうとしたり、話を元に戻そうとしたりした場合は、それに素直に合わせてください。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
  }
  ...
  if (Math.random() < 0.5) {
    ...
    if (picked) {
      return `${scene}\n\n直前の${counterpart}の発言に軽く区切りをつけつつ、新しい話題として、最近パンタローネ自身が対話した「${picked.displayName}」について、以下の実際の記録に基づいた内容を一言持ちかけてください：\n` +
        `「${picked.memoryText}」\n` +
  ...
  return `${scene}\n\n直前の${counterpart}の発言に軽く区切りをつけつつ、新しい話題として、博士の最近の実験・研究について、興味や皮肉を交えて一言尋ねるか触れてください。具体的な実験内容は自由に創作していただいて構いません。1〜3文程度でお願いします。普段の会話と同じ形式（括弧書きの動作描写を交えても構いません）で、セリフ本文を出力してください。`;
}
